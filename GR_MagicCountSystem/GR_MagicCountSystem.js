...
// 独自関数 ======================================================================
    ...
    let getMagicLevel = function(skill) {
        ...
        if(type == -1) return -1;
        let level = Math.floor(((skill.id - START_ID) - (SPELLS_PER_LEVEL * MAX_LEVEL + 1) * type) / SPELLS_PER_LEVEL) + 1;
        return level;
    };
...
// Game_Battlerに各処理、プロパティ等を追加 =============================================
    ...
    Game_Battler.prototype.isLearnedMagic = function(type ,level) {
        let result = false;
        let tempSkillId = START_ID + type * (SPELLS_PER_LEVEL * MAX_LEVEL + 1) + (level - 1) * SPELLS_PER_LEVEL;
    ...
    Game_Battler.prototype.maxMagicCalc = function(type) {
        ...
        for (let i = 0; i < MAX_LEVEL; i++) {
            ...
            if(this.isLearnedMagic(type, i + 1)) {
                tempNum = Math.floor((this.param(CALCULATION_BASE_PARAMID) * ((CALCULATION_COEFFICIENT - i - 1) * 0.01 + 0.24)  + CALCULATION_BIAS - (i - 1) * 1.6) * aptitude);
...
// Game_Enemy側の各種処理 ==========================================================
    ...
    //スキルコストの確認をオーバーライド
    let _game_enemy_canPaySkillCost = Game_Enemy.prototype.canPaySkillCost;
    Game_Enemy.prototype.canPaySkillCost = function(skill) {
        let checkMpTp = _game_enemy_canPaySkillCost.call(this, skill);
        let type = getMagicType(skill);
...
//メニュー画面=====================================================================
    ...
    Window_SkillList.prototype.checkSkillLevel = function(item) {
        if(item) {
            ...
            let scopeStart = START_ID + (SPELLS_PER_LEVEL * MAX_LEVEL + 1) * type + (skillLevel - 1) * SPELLS_PER_LEVEL;
